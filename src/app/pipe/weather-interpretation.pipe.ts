...
export class WeatherInterpretationPipe implements PipeTransform {
    ...
    transform(code: number,  type: 'description' | 'icon'): string {
        const weatherInterpretation: { [key: number]: { description: string, icon: string } } = {
          0: { description: 'Clear sky', icon: 'wi-day-sunny' }, // ou 'wi-night-clear' selon l'heure
        ...
        const result = weatherInterpretation[code] || { description: 'Unknown', icon: 'wi-na' };
